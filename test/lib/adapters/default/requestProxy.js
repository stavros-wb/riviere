...
it('defaultAdapter.requestProxy should default the arg to empty obj', () => {
  // todo
  defaultAdapter.requestProxy();
});

it('defaultAdapter.requestProxy should pass when incomingMessage.url exists', () => {
  const logger = {
    info: sandbox.spy(),
...
it('defaultAdapter.requestProxy should pass when no protocol', () => {
  const logger = {
      info: sandbox.spy(),
  };
  const serialize = a => a;
  ...
  const incomingMessage = {
    ...
    const httpRequest = function () {
        ...
        arguments[0].should.eql({ method: 'GET',
            port: '8080',
            headers: { test: 'ok' },
            url:
                { protocol: undefined,
...
it('defaultAdapter.requestProxy should pass when incomingMessage.url does not exist', () => {
  const logger = {
    info: sandbox.spy()
  };
  const serialize = a => a;
  ...
  http.request(incomingMessage);
    logger.info.callCount.should.eql(2);
    logger.info.args[0][0].should.eql({ method: 'GET',
        protocol: 'https',
        host: undefined,
        port: '8080',
        path: '/some?somequery=query',
...
it('defaultAdapter.requestProxy should pass when requestId does not exist', () => {
  const logger = {
    info: sandbox.spy()
  };
  const serialize = a => a;
  ...
  http.request(incomingMessage);
    ...
    logger.info.args[0][0].should.eql({ method: 'GET',
      ...
      requestId: undefined,
      log_tag: 'outbound_request' });
  logger.info.args[1][0].should.eql({ method: 'GET',
      path: '/some?somequery=query',
      status: 200,
      duration: 0,
      requestId: undefined,
...
it('defaultAdapter.requestProxy should pass when requestId key is not given', () => {
  const logger = {
    info: sandbox.spy()
  };
  const serialize = a => a;
...
it('defaultAdapter.requestProxy should pass if the proxy throws an error before proxying', () => {
  const logger = {
    info: sandbox.spy(),
    error: sandbox.spy()
  };
...
it('defaultAdapter.requestProxy should pass if the proxy throws an error after proxying', () => {
  const logger = {
    info: sandbox.spy(),
    error: sandbox.spy()
  };
