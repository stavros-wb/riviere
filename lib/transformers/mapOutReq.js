...
module.exports = (incomingMessage, reqId = undefined, opts = {}) => {
  ...
  const shouldSliceQuery = !!(maxLength && query && query.length > maxLength);
  const shouldSlicePath = !!(maxLength && path && path.length > maxLength);
  const shouldSliceHref = !!(maxLength && href && href.length > maxLength);

  const slicedQuery = shouldSliceQuery ? query.slice(0, maxLength) + '...' : query;
  const slicedPath = shouldSlicePath ? path.slice(0, maxLength) + '...' : path;
  const slicedHref = shouldSliceHref ? href.slice(0, maxLength) + '...' : href;
